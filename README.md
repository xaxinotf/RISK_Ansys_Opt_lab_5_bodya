...
Я зрозумів 👍 У тебе GitHub README не відображає формули так, як у LaTeX-документах. На GitHub **Markdown не підтримує `$$ ... $$` напряму**, але можна:

1. Використати **inline/блочні формули у синтаксисі KaTeX/MathJax**, які GitHub рендерить через *GitHub Markdown math support* (але це працює не скрізь).
2. Або обійтись як у більшості проєктів — **написати формули як код** (` ```math ... ``` `) або через `\[` ... `\]`.

Тобі підійде ось таке рішення для README (яке точно рендериться правильно):
...
```math
Score_i^{borda} = \sum_k w_k \cdot r_{ik}
````

де **менше — краще**.
Для зручності візуалізації нормуємо до інтервалу $[0,1]$.
...
```math
Score_i^{avg} = \sum_k w_k \cdot \tilde{r}_{ik}, \quad \text{менше — краще}
```
...
За матрицею рангів $R \in \mathbb{R}^{n\times m}$
($n$ — банки, $m$ — індикатори):

```math
W = \frac{12 \sum_{i=1}^n (\bar{R}_i - \bar{R})^2}{m^2 (n^3 - n)}, \quad W \in [0,1]
```

* $W \approx 1$ — індикатори дуже **узгоджені**;
* $W \approx 0$ — індикатори **хаотичні**.
